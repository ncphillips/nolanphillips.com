  ...
  return (
    <Dialog {...props}>
      {({ titleProps, dialogProps, underlayProps, dialogRef }) => (
        <DialogModalContext
          value={{ id: dialogProps["aria-labelledby"]!, parent: parentId }}
        >
          <div
            className="flex flex-col items-center justify-center fixed top-0 right-0 bottom-0 left-0 bg-[rgba(0,0,0,0.2)]"
            {...underlayProps}
          >
            <ScrollHandler>
              {({ modalProps }) =>
...
const ScrollHandler = ({
  ...
  children(modalProps: any): ReactNode;
}) => {
